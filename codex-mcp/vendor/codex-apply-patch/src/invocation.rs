...
pub async fn maybe_parse_apply_patch_verified(
    ...
    maybe_parse_apply_patch_verified_with_mode(argv, cwd, ApplyPatchFileUpdateMode::default(), fs)
        .await
}
...
pub async fn maybe_parse_apply_patch_verified_with_mode(
    ...
    match maybe_parse_apply_patch(argv, cwd) {
        MaybeApplyPatch::Body(args) => {
            verify_apply_patch_args_with_mode(args, cwd, update_file_mode, fs).await
        }
...
pub async fn verify_apply_patch_args(
    ...
    verify_apply_patch_args_with_mode(args, cwd, ApplyPatchFileUpdateMode::default(), fs).await
}
...
pub async fn verify_apply_patch_args_with_mode(
    ...
    match try_verify_apply_patch_args(args, cwd, update_file_mode, fs).await {
...
async fn try_verify_apply_patch_args(
    ...
    for hunk in hunks {
        ...
        match hunk {
            ...
            Hunk::DeleteFile { .. } => {
                let content =
                    fs.read_file_text(&path, Default::default())
                        .await
                        .map_err(|source| {
            ...
            Hunk::UpdateFile {
                ...
                let ApplyPatchFileUpdate {
                    ...
                    content: contents,
                    ..
                } = unified_diff_from_chunks_with_mode(&path, &chunks, update_file_mode, fs)
                    .await?;
                changes.insert(
                    path,
...
"#,
        ...
        let diff = unified_diff_from_chunks(&path_uri, chunks, LOCAL_FS.as_ref())
            .await
            .unwrap();
...
+after"#
    ...
    async fn test_unreadable_destinations_still_verify() {
        ...
        for argv in [add_argv, move_argv] {
            let result = maybe_parse_apply_patch_verified(&argv, &cwd, LOCAL_FS.as_ref()).await;
