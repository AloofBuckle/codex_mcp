...
fn native_browser_url(args: &[String]) -> Result<Option<String>> {
    if args.len() > 1 {
        bail!("native-app browser accepts at most one URL")
    }
    let Some(raw) = args.first() else {
        return Ok(None);
    };
    let parsed = url::Url::parse(raw).with_context(|| format!("invalid browser URL {raw:?}"))?;
    if !matches!(parsed.scheme(), "http" | "https" | "file") {
        bail!("native browser URL must use http, https, or file")
    }
    Ok(Some(parsed.to_string()))
}

fn native_app(config: &LoadedConfig, name: &str, extra_args: &[String]) -> Result<()> {
    match name {
        "terminal" => {
            if !extra_args.is_empty() {
                bail!("native-app terminal does not accept extra arguments")
            }
            native_launch_app(
                config,
        ...
        "files" => {
            if !extra_args.is_empty() {
                bail!("native-app files does not accept extra arguments")
            }
            native_launch_app(
                config,
        ...
        "browser" => {
            let url = native_browser_url(extra_args)?;
            if let Some(port) = native_browser_debug_port(config)? {
                ...
                native_launch_app(config, get_string(&config.value, "tools.node")?, args)
            }
        }
...
fn deploy(config: &LoadedConfig, destination: &Path) -> Result<Vec<PathBuf>> {
    ...
    if get_bool(&config.value, "nativePlasma.enabled")? {
        ...
        let exec = format!("{} native-app browser --config {} %u", exe.display(), cfg_s);
...
fn native_install(config: &LoadedConfig, apply: bool, start: bool) -> Result<()> {
    ...
    if get_bool(&config.value, "nativePlasma.enabled")? {
        ...
        let xdg_mime =
            find_executable("xdg-mime").ok_or_else(|| anyhow!("xdg-mime is required"))?;
        let envs = native_environment(config)?;
        for mime in [
            "x-scheme-handler/http",
            "x-scheme-handler/https",
            "text/html",
        ] {
            let mut query = Command::new(&xdg_mime);
            query.args(["query", "default", mime]).envs(&envs);
            let current = run_output(query, "query Native browser MIME default")?;
            if current.trim().is_empty() {
                let mut set = Command::new(&xdg_mime);
                set.args(["default", "google-chrome.desktop", mime])
                    .envs(&envs);
                run_status(set, "set Native browser MIME default")?;
            }
        }
    }
