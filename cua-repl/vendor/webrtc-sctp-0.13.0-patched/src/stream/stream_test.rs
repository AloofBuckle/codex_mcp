...
#[tokio::test]
async fn regression_shutdown_releases_blocked_writer() {
    let (wake, _rx) = mpsc::channel(1);
    let queue = Arc::new(PendingQueue::with_limit(1024));
    let s = Arc::new(Stream::new("close-regression".into(), 0, 1024,
        Arc::new(AtomicU32::new(65536)),
        Arc::new(AtomicU8::new(AssociationState::Established as u8)),
        Arc::new(wake), queue));
    s.write(&Bytes::from(vec![0; 1024])).await.unwrap();
    let writer = s.clone();
    let task = tokio::spawn(async move { writer.write(&Bytes::from_static(b"blocked")).await });
    tokio::task::yield_now().await;
    assert!(!task.is_finished());
    s.shutdown(Shutdown::Write).await.unwrap();
    let result = tokio::time::timeout(std::time::Duration::from_millis(200), task).await;
    assert!(result.is_ok(), "stream shutdown left writer pending forever");
