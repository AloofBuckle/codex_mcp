...
impl Association {
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        // No lock or network wait may precede waking queue waiters: a stream
        // reset can itself be waiting for this queue under the association lock.
        self.pending_queue.close();
        { let mut ai = self.association_internal.lock().await; ai.close().await?; }
    ...
    async fn new(config: Config, is_client: bool) -> Result<(Self, mpsc::Receiver<Option<Error>>)> {
        ...
        let pending_queue = Arc::clone(&ai.pending_queue);
