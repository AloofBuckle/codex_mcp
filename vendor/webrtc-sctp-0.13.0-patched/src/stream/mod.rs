...
pub struct Stream {
    ...
    pub(crate) write_notifier: Notify,
    write_serial: Mutex<()>,
...
impl Stream {
    ...
    pub async fn write_sctp(&self, p: &Bytes, ppi: PayloadProtocolIdentifier) -> Result<usize> {
        let closed = self.write_notifier.notified();
        tokio::pin!(closed);
        closed.as_mut().enable();
        tokio::select! { biased;
            _ = &mut closed => Err(Error::ErrStreamClosed),
            result = async {
                // Allocate SSNs in the same order in which writes are committed.
    ...
    pub async fn shutdown(&self, how: Shutdown) -> Result<()> {
        ...
        if how == Shutdown::Write || how == Shutdown::Both {
            ...
            self.write_notifier.notify_waiters();
        }
    ...
    async fn send_payload_data(&self, chunks: Vec<ChunkPayloadData>) -> Result<()> {
        ...
        self.pending_queue.append_and_notify(chunks, || self.awake_write_loop()).await?;
